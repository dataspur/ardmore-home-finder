...
-- Drop and recreate get_lease_details with enhanced data
DROP FUNCTION IF EXISTS public.get_lease_details(uuid);
...
CREATE OR REPLACE FUNCTION public.get_tenant_payment_history(lookup_token uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  result JSON;
BEGIN
  SELECT json_agg(payment_row) INTO result
  FROM (
    SELECT 
      p.id,
      p.amount_cents,
    ...
    FROM tenants t
    INNER JOIN leases l ON l.tenant_id = t.id
    INNER JOIN payments p ON p.lease_id = l.id
    WHERE t.access_token = lookup_token
    ORDER BY p.created_at DESC
    LIMIT 5
  ) as payment_row;
